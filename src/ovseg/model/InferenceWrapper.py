...
def evaluate_segmentation_model(im,
    ...
    # stored
    path_to_model = os.path.join(project_root,'data', 'clara_models', model)
    ...
    path_fallback = os.path.join(project_root,  'preprocessing_parameters.pkl')


    if os.path.exists(path_priority):

        path_to_preprocess_params = path_priority
    else:

        path_to_preprocess_params = path_fallback


    if not os.path.exists(path_to_preprocess_params):
        raise FileNotFoundError("Impossible de trouver 'preprocessing_parameters.pkl' dans aucun des deux emplacements.")
