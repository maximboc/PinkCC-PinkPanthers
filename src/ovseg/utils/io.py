...
try:
    ...
    print(
        "Caught Import Error while importing some function from scipy or skimage. "
...
def read_data_tpl_from_nii(folder, case):
    ...
    if not exists(folder):
        raise FileNotFoundError(
            "Can't read from folder {}. It doesn't exist.".format(folder)
        )
    ...
    image_folders_ex = [
        join(folder, imf) for imf in possible_image_folders if exists(join(folder, imf))
    ]
    ...
    for image_folder in image_folders_ex:
        matching_files = [
            join(folder, image_folder, file)
            for file in listdir(join(folder, image_folder))
            if file.startswith(case[:-7])
        ]
        if len(matching_files) > 0 and len(image_files) > 0:
            raise FileExistsError(
                "Found images for in multiple image folders at path {} for "
    ...
    if len(image_files) == 0:
        ...
        if not np.all([np.all(spacings[0] == sp) for sp in spacings[1:]]):
            raise ValueError(
                "Found unequal spacings when reading the image files {}"
                "".format(image_files)
            )
    ...
    label_folders_ex = [
        join(folder, lbf)
        for lbf in ["labels", "labelsTr", "labelsTs"]
    ...
    for label_folder in label_folders_ex:
        matching_files = [
            join(folder, label_folder, file)
            for file in listdir(join(folder, label_folder))
            if file.startswith(case[:-7])
        ]
        if len(matching_files) > 0 and len(label_files) > 0:
            raise FileExistsError(
                "Found labels for in multiple label folders at path {} for "
                "case {}.".format(folder, case)
            )
...
def read_dcms(dcm_folder, reverse=True, names_dict=None, dataset=None):
    ...
    if len(roidss) > 1:
        raise FileExistsError(
            "Found multiple ROI dcms in folder " + dcm_folder + ". "
    ...
    if len(roidss) == 1:
            ...
            if np.all([name[0].isdigit() for name in names_found]):
                ...
                for name in names_found:
                    if not name[:1].isdigit():
                        raise ValueError(
                            "Could not perform automated numbering of ROIs "
                            + name
                ...
                if not _names_dict_warning_printed:
                    print(
                        "Warning: No names_dict was found and the ROIs do not start with integer "
        ...
        for i in range(len(roids.ROIContourSequence)):
            ...
            for s in roids.ROIContourSequence[i].ContourSequence:
                ...
                if np.max(ad) > 0.1:
                    ValueError(
                        "z axis of difference larger than .1mm found"
    ...
    ds = imdss[0]
    for key, attr in zip(
        ["pat_id", "date", "pat_name"], ["PatientID", "AcquisitionDate", "PatientName"]
    ):
        if hasattr(ds, attr):
...
def save_nii(arr, tarp, srcp):
    ...
    assert (
        arr.ndim == 3
    ), "Array must be 3d (or 4d with leading dim 1) to be stored as nifti."

    arr = np.rot90(arr, 3, (1, 2))[::-1]
    arr = np.moveaxis(arr, 0, 2)

    orig_nii_img = nib.load(srcp)
    new_nii_img = nib.Nifti1Image(arr, orig_nii_img.affine)
        ...
        if exists(data_tpl["raw_image_file"]):
            ...
            for imf in im_folders:
                if basename(data_tpl["raw_image_file"]) in listdir(join(raw_path, imf)):
                    im_file.append(
                        join(raw_path, imf, basename(data_tpl["raw_image_file"]))
                    )
...
def save_dcmrt_from_data_tpl(
    data_tpl, out_file, key, names=None, colors=None, dcm_path=None
):

    dcm_path = data_tpl["raw_image_file"] if dcm_path is None else dcm_path
    if not isdir(dcm_path):
        raise FileNotFoundError(
            "No path with dcms was given for the export to dcmrt. If "
    ...
    for i, (name, color) in enumerate(zip(names, colors)):
        ...
        if mask.max() > 0:
            ...
            rtstruct.add_roi(
                mask=mask[..., ::-1], color=color, name=name, approximate_contours=False
            )
